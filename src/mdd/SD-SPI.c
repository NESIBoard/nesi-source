...
//prescalers cannot reach a low enough SPI frequency.  Therefore, we initialize
...
BYTE MDD_SDSPI_SectorRead(DWORD sector_addr, BYTE* buffer)
{
    ...
    //Impossible to get here, but we will return a value anyway to avoid possible
    //compiler warnings.
    return FALSE;
}
    ...
    switch(info->bStateVariable)
    {
        ...
        case ASYNC_READ_NEW_PACKET_READY:
            ...
            if(ioInfo.dwBytesRemaining != 0x00000000)
            {
                ...
                //This operation directly dictates data throughput in the
...
/*****************************************************************************
  ...
  Summary:
    ...
    at high speed for optimum throughput, and copies them into the user
...
#endif
{
    ...
    //SPI module initialization depends on processor type
    #ifdef __PIC32MX__
...
/*****************************************************************************
  ...
  Input:
    Uses GetSystemClock() macro value.  Should be #define in the hardware profile.
  Return Values:
...
MEDIA_INFORMATION *  MDD_SDSPI_MediaInitialize(void)
{
	...
	for (count = 0; count < 20u; count ++)
	{
		...
		index++;
		/* Hopefully the first byte is the data token, however, some cards do
		not send the response token before the CSD register.*/
		if((count == 0) && (CSDResponse[0] == DATA_START_TOKEN))
		{
			/* As the first byte was the data token, we can drop it. */
			index = 0;
		}
	}
